 ...
 * credentials, and nothing else — no knowledge of streams, pipelines, or which nodes are
 * live, and it no longer owns node ports (several nodes per VM share a host and differ by
 * port, so the port belongs to the node). Callers (the `media-nodes` feature) resolve
 * *which* host:port to talk to from the node registry and hand them in (ARCH-09/ARCH-10).
...
export class MediaMtxClientRegistry {
    ...
    getClient(host: string, port: number, role: NodeRole): MediaMtxClient {
    ...
    getMetricsClient(host: string, port: number, role: NodeRole): MediaMtxMetricsClient {
        return this.metricsFactory.getOrCreate(buildNodeUrl(this.authFor(role), host, port));
    }

    private authFor(role: NodeRole): string {
        return authPrefix(
            role === NodeRole.INGEST ? this.config.ingestNodeAuth : this.config.clusterNodeAuth,
        );
    }
}
