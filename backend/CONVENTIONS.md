...
**ARCH-04** — Rule: No circular dependencies between features. If two features need each other, the shared part moves to `common/` or communication switches to events. (`forwardRef` is a last resort. The `MediaMtxModule` ↔ `PodsModule` **module-seam** forwardRef was eliminated by ARCH-08: persistence is composed in `DatabaseModule` and imported by feature modules via the `@/infrastructure/database` sub-barrel, so `PodsModule` no longer reaches the `@/infrastructure` barrel — the barrel cycle that left `PodsModule` undefined at scan time is gone (confirmed by a boot DI-scan). The forwardRefs that remain are **service-level** — `@Inject(forwardRef(() => PodQueryService))` in the media-mtx infra services (`ClusterNodeResolverService`, `IngestStreamListingStrategy`, `MediaMtxMetricsService`), infra injecting a pods service. They share the same now-broken cycle root and are probably vestigial, but removing them requires a live-Mongo boot to confirm — see [ADR-0008](../docs/adr/0008-runtime-safe-barrel-imports.md), [ADR-0009](../docs/adr/0009-pod-derived-cluster-topology.md).)
...
Example: `MediaMtxClientRegistry` is imported as `../../registry` inside `infrastructure/media-mtx/services/`, never as `@/infrastructure`; `PodsModule` imports `DatabaseModule` from `@/infrastructure/database` (not `@/infrastructure`), which is what lets `MediaMtxModule` import `PodsModule` directly with no `forwardRef`. The surviving forwardRefs are the `@Inject(forwardRef(() => PodQueryService))` infra→pods service injections (ARCH-04).
