...
**PHIL-01** — Rule: Code MUST be organized by feature/domain (streams, nodes, alerts, metrics, stream-inspection, sync), not by technical layer at the top level. Layers exist _inside_ each feature.
...
**PHIL-08** — Rule: Two features that play mirror roles (ingest vs cluster) SHOULD read parallel — same layer/folder names, same verb families, same call shapes — so a reader who knows one can read the other. But structural symmetry is the **default, not a mandate**: where the job genuinely differs, the code MUST differ, and the difference MUST be statable in one sentence. Force-mirroring a real asymmetry is as much a defect as gratuitous divergence. The deciding test, applied both ways: can you name, in one sentence, why this process diverges from its twin? If yes, the divergence is earned; if no, converge them.
Example: cluster assignment *mutates an existing record* (`StreamAssignmentService.ensureAssigned` persists `assignedNode`, idempotent), while ingest placement is a *birth-time input folded into the reservation insert* (`IngestPlacementService.selectNode` feeds `createReservation`, no persist step) — different jobs, so they legitimately do not mirror, and INT-06 records the reason. Conversely, the media-mtx listing's ingest/cluster "strategies" had no such reason and were converged into `listStreams(role)`.
Enforced: review.

**PHIL-09** — Rule: An abstraction (strategy/policy/abstract base/variant folder) is allowed only once you can name the **concrete second implementation** that justifies it. No real second instance today and none concretely arriving → it is speculation; collapse to a method or a pure function (SVC-02, DIR-06). "Why is this a strategy?" must be answerable with an existing sibling, never "in case". The counterweight is not "never abstract" but "abstract on the second instance, not the first".
Example: the djb2 hash pick was a lone `StreamAssignmentPolicy` behind an abstract token with one implementation forever — collapsed to the pure `selectByHash`. Contrast the two legitimate `extends` in the tree (PHIL-07), each with a real subclass hole.
Enforced: review.
...
**TOOL-07** — Rule: Application code uses `async/await`, not `.then`/`.catch` chains, for sequential async flow — a value threaded through several `.then`s reads as callback nesting and hides control flow. A single fire-and-forget `.catch` on a promise you deliberately don't await is fine; a *chain* that sequences work is not.
Example: `IngestActivationController.streamReady` awaits `synchronizer.activate(...)` rather than returning a `.then(() => ({ accepted: true }))` chain.
Enforced: review.
...
| `.controller.ts` | HTTP entry point                                | `nodes/controllers/nodes.controller.ts`                                      |
...
Example: `NodeResolver` vends one `get*` family — `getIngestClient`, `getActiveIngestClients`, `getActiveClusterClients`, `getClusterClientForNode`, `getMetricsTargets` — so the accessors read as a set; the gateway `MediaMtxClientRegistry` mirrors it with `getClient`/`getMetricsClient`.
...
**NAME-07** — Rule: New event names MUST be added to `SystemEventNames` in `common/domain/consts/system-event-names.const.ts` using the `<subject>.<verb-past>` dotted form (`stream.synced`, `alert.created`, `node.registered`). Never emit a string literal.
...
**NAME-09** — Rule: A name carries at most two independent qualifier axes: the **tier/entity** it operates on (`node`; the ingest/cluster *role*) and the **action/job** it performs (placement, assignment, reservation, synchronization). The axes are orthogonal — changing one MUST NOT collapse the other. Renaming a tier (`pod`→`node`) does not merge two distinct actions; two names that differ by *action* (`IngestPlacementService` vs `StreamAssignmentService`) stay distinct because the jobs differ, not because a tier word does. Conversely, if two names differ only by a **tier word** for the **same job**, unify the job name and make the tier a **parameter**, not a name fragment: `getNodeLoads(role)`, `clientForNode(role, id)`, `listStreams(role)` — never `getIngestLoads`/`getClusterLoads` twins.
Example: the media-mtx listing, metrics, and topology ops are role-parameterized (one method, `role` argument); placement and assignment keep separate names because they are separate jobs (INT-06, PHIL-08).
Enforced: review.

**NAME-10** — Rule: An asymmetric qualifier is a smell. A qualifier that appears on one half of a conceptual pair with no counterpart on the other (`ingest*` on everything, no `cluster*`) means one of two things, and you MUST pick: either both sides deserve the parallel qualifier (add the twin), **or** the operation is genuinely single-tier — it exists for one tier only and has no analog — and the one-sided name is correct (`getIngestRtspUrl` has no cluster equivalent; the relay only ever pulls *from* ingest). Do not leave a prefix scattered lopsidedly across a feature as if it were decoration.
Example: `getIngestRtspUrl` is legitimately single-sided (tier-specific op); the old `ingest`/`cluster` listing "strategy" prefixes were not (same op both tiers) and were unified to `listStreams(role)`.
Enforced: review.
...
Example: `alerts/domain/types/{alert/, rules/{metric,stream-track,node}/}` — `Alert` plus its create/update payloads under `alert/`; each producer's rule alias and context under `rules/<subject>/`. Likewise `infrastructure/media-mtx/types/{v3/, stream/, metrics/}` + flat `pipeline-create-result.types.ts` and `media-mtx-node-endpoint.types.ts` — raw wire shapes (`V3PathItem`/`V3TrackItem`/`V3PathSource`) grouped under `v3/`, domain output shapes (`MediaMtxStreamInfo`/`StreamDetails`/`ContextualMediaMtxStream`/`StreamPathMetadata`) under `stream/`, scrape shapes (`PrometheusSample`/`MediaMtxMetricsSnapshot`) under `metrics/`, and the two lone single-theme shapes left flat — one shape per file, each subject folder with its own barrel.
Enforced: review.

**DIR-11** — Rule: A new capability that parallels an existing one lands in the **existing concern folders** beside its twin — it does NOT grow a parallel folder tree prefixed by its own name. The `services/` taxonomy is by **concern** (assignment, listing, orchestration, workflow), not by feature-instance; a feature-name prefix on a folder (`ingest-assignment/`, `ingest-sync/`) is the smell. Ingest placement sits in `streams/services/assignment/` next to cluster assignment; ingest synchronization sits in `sync/services/workflows/` next to the other workflows. This is DIR-04/DIR-06 applied to a second *instance* of a concern rather than a second concern: reuse the layer, don't fork it.
Example: `IngestPlacementService` under `assignment/`, `IngestStreamSynchronizerService` under `workflows/` — the `ingest-*` sibling folders that first held them were removed.
Enforced: review.
...
**ARCH-04** — Rule: No circular dependencies between features. If two features need each other, the shared part moves to `common/` or communication switches to events. `forwardRef` is a last resort and the tree currently uses **none** (`grep -rn forwardRef src/` is empty). Two rounds removed them: (1) the `MediaMtxModule` ↔ `NodesModule` **module-seam** forwardRef was eliminated by ARCH-08 — persistence is composed in `DatabaseModule` and imported by feature modules via the `@/infrastructure/database` sub-barrel, so `NodesModule` no longer reaches the `@/infrastructure` barrel and the barrel cycle that left `NodesModule` undefined at scan time is gone; (2) the three media-mtx infra services that injected `NodeQueryService` (`IngestStreamListingStrategy`, `MediaMtxMetricsService`, and the now-deleted `ClusterNodeResolverService`) had that dependency **inverted out entirely** per ARCH-09 — infra no longer imports `@/nodes` at all, so `MediaMtxModule` dropped its `NodesModule` import and now initializes in the eager batch (before the Mongo gate), independent of nodes. Confirmed by a boot DI-scan (all synchronous DI resolves; boot fails only on the Mongo connection) and the full test suite. See [ADR-0008](../docs/adr/0008-runtime-safe-barrel-imports.md), [ADR-0009](../docs/adr/0009-pod-derived-cluster-topology.md).
...
**ARCH-09** — Rule: An `infrastructure/` adapter MUST NOT depend on a feature — the dependency arrow runs feature → infrastructure, never back (PHIL-04). When an adapter needs domain state to do its job (which nodes are live, which node a stream is assigned to), it does **not** reach up into the feature that owns that state; the feature **resolves the state and passes it in as a parameter**. The adapter stays pure transport, taking a plain data shape (a host string, a role) it defines itself, so any feature can call it and none is imported. Parameter-passing is the fix when the caller is a **genuine adapter operation**. But if the thing reaching for `@/nodes` is really application logic mis-filed under `infrastructure/`, the deeper fix is ARCH-10: move it to a feature, where it may inject `NodeQueryService` directly (feature → feature) and self-resolve — no threading. That is what happened here: the media-mtx orchestration services were extracted to the `media-nodes` feature, so `MediaMtxPipelineService.buildClusterPullPipeline(stream, assignedNodeId)`, `MediaMtxStreamStatsService`, and `MediaMtxMetricsService.collect()` resolve nodes internally (via `NodeResolver`), and the transport-only `MediaMtxClientRegistry` (the true adapter) took an even smaller contract: **a single node host + role**, `getClient(host, role)` / `getMetricsClient(host, role)` — not even an endpoint array. The feature resolves the live host from the node registry and hands it in one at a time; the adapter is addressless (see ARCH-11).
Enforced: review + boot DI-scan (a violating adapter drags the feature's module into its graph).
Example: the gateway `MediaMtxClientRegistry.getClient(host, role)` turns a host into a cached client (pure transport); the `media-nodes` feature's `NodeResolver` injects `NodeQueryService` + the registry and bridges "which nodes are active" to "give me a client for this host" (for ingest and cluster).

**ARCH-10** — Rule: Separate an integration's **gateway** (the driver that talks to the external system) from the **application layer** that operates it. The gateway lives in `infrastructure/<system>/` and holds only transport: HTTP clients, their caching factories, the registry that vends them, boundary mappers, and the raw wire types — all mapping happens *inside the client* so raw shapes never leave the gateway. The orchestration that *uses* the gateway to do something meaningful for this system — discovery strategies, resource lifecycle (create/delete), stat/metric collection, role/context tagging — is **application logic and belongs in a feature**, not under `infrastructure/`. The tell that logic is mis-filed as "infra": it needs domain state (which nodes are live) or makes system-specific decisions (fallback order, already-exists semantics) — pure transport never does, and mis-filing it is what forces an adapter to import a feature (ARCH-09). A feature over a gateway is a normal feature: it imports the gateway module, injects the registry, and injects whatever domain features it needs.
...
Enforced: review + boot DI-scan.
Example: `DatabaseModule` binds all six repositories; `NodesModule` imports it and drops its own `forFeature` + adapter provider.

**ARCH-11** — Rule: Runtime topology has **one source of truth**, and a transport adapter is not it. The set of live nodes (which nodes exist, their hosts, their health) belongs to the feature that owns that state — here the node registry (`NodesModule`), fed by heartbeats. The transport adapter is **addressless**: it turns a caller-supplied host into a client using only per-deployment transport config (port, credentials) and never holds node addresses or decides which node to hit. Config's job is *transport parameters* (ports, auth, timeouts), **not** a parallel list of node addresses. A static-address fallback in the adapter ("use the configured URLs when no nodes are registered") is a second, stale source of truth: it silently targets a hardcoded node when the real answer is "none are live", masking outages the same way a nullable-and-defaulted precondition does (SVC-08). Remove it — an empty live set means no client, and the **feature** decides what that means (skip, empty result, or throw), not the adapter. The rule governs addressing a **specific, dynamically-changing node instance**. It does **not** apply to a **stable shared endpoint** — a Service/DNS, a load balancer, a database — which fronts the instances and is legitimately deployment config (CFG-01), not node discovery. Deciding test: *are you talking to one specific live node, or to "the system" through a stable front?* Specific node → node registry; stable front → config.
...
Enforced: review + boot DI-scan.
Example: `MediaMtxClientRegistry` was cut from owning `ingestBaseUrl`/`clusterBaseUrls` + a `nodes.length ? fromNodes : staticPool` fallback + round-robin, down to `getClient(host, role)`/`getMetricsClient(host, role)` — host in, cached client out, reading config only for port + `ingestNodeAuth`/`clusterNodeAuth`. `NodeResolver` (feature) now owns every addressing decision: it asks `NodeQueryService` for live nodes and, finding none, returns `[]` or throws (`No active cluster node for node …`) rather than hitting a seed URL. The old `INGEST_MEDIAMTX_BASE_URL`/`CLUSTER_MEDIAMTX_BASE_URL(S)` env vars were deleted; credentials that used to be parsed out of them became explicit `*_MEDIAMTX_AUTH` transport config. Counter-example (the stable-front exception): `NodeResolver.getIngestPullUrl` builds the cluster→ingest RTSP pull URL from `config.ingestRtspBaseUrl` (`INGEST_RTSP_URL`, a Service/DNS like `rtsp://mediamtx-ingest:8554`) — the media plane pulls from "the ingest" through a stable relay, not a specific ingest node, so this address stays config while the control-plane per-node HTTP addressing is node-derived.
...
**TYPE-01** — Rule: Fixed value sets are TypeScript enums in `common/domain/enums/` when shared (`StreamStatus`, `NodeRole`, `NodeStatus`, `AlertType`, `AlertSeverity`, `TrackType`) or in the feature's `domain/enums/` when private. Never raw string unions for domain states.
...
**TYPE-04** — Rule: Multi-parameter data is passed as a named structure, not positional primitives, when it crosses module boundaries (e.g. `NodeRegistrationData`, `CreateStreamData`, `SyncContext`).
...
**DTO-03** — Rule: DTO names follow `<Action><Entity>Dto` (`CreateStreamDto`, `UpdateStreamDto`, `AssignStreamDto`, `RegisterNodeDto`, `HeartbeatDto`).
...
- **query** — reads, no side effects (`StreamQueryService`, `NodeQueryService`)
...
Example: the alerts pipeline — metrics/inspection/nodes _produce_ data events, rulers in `alerts/services/rulers/` _evaluate_ them into `AlertSignal[]`, and `AlertReconcileService` _reconciles_ by `(source, subject, type)`. Decision history: [ADR-0010](../docs/adr/0010-event-driven-alert-pipeline.md), [ADR-0011](../docs/adr/0011-node-resource-alerts-third-producer.md).
Enforced: review.
...
Example: `buildClusterPullPipeline(stream, assignedNodeId: string)` takes a **required** node id (was `?: string | null`); the precondition is enforced once in `StreamPipelineService.build`, which throws `Cannot build cluster pipeline for unassigned stream ${name}` for an unassigned stream — surfacing a latent bug where `StreamReconcileService` passed a stale, un-assigned stream and the nullable design silently round-robined instead of erroring. Likewise `MediaMtxStreamStatsService` split its single `getStreamDetails(name, role, assignedNodeId?)` into `getIngestStreamDetails(name)` and `getClusterStreamDetails(name, assignedNodeId: string)` — no nullable, no internal role re-branch; the inspection caller (tolerant by design) turns a cluster stream that lacks an assignment into recorded `lastError`, never a wrong-node scrape. Runtime "the world moved" cases are decided deliberately, not defaulted: `NodeResolver.getClusterClientForNode(nodeId: string)` takes a required node id and, when that node is no longer in the live set, **throws** `No active cluster node for node ${nodeId}` — the assigned node genuinely being gone is surfaced (the caller's deploy/sync path records it), never silently rerouted to a random node. (An earlier round-robin-over-a-static-pool fallback here was removed with the static addressing itself — ARCH-11.)
Enforced: review.

**SVC-09** — Rule: Service boundaries follow **dependency cohesion**. Group methods into one service by their shared collaborator set; split a service when a method's dependencies are largely **disjoint** from its siblings'. A method that injects collaborators none of its neighbors touch is a seam — extract it into its own service carrying only what it needs. Converse holds: do not split methods that share most of their dependencies just to shrink a file (that scatters one job across classes that must be wired back together). Judge by the constructor: if half the injected deps exist to serve one method, that method belongs elsewhere.
Example: `reserve` was pulled out of `StreamSetupService` into `StreamReservationService` because it needed `IngestPlacementService` + config + token minting while `onboard` needed `StreamPipelineService` + `StreamStatusService` — the two shared almost no dependencies. The stable seam this produced also satisfies SVC-01 (reservation vs setup are distinct roles).
Enforced: review.

**SVC-10** — Rule: A point event gets a **targeted** operation, not a full-cycle sweep. When a single subject becomes actionable (one stream ready on one node), act on that subject directly with bounded work; do NOT reuse the periodic whole-population reconcile as the handler. The periodic sweep exists for drift and catch-up over *everything*; running it to service one known subject re-scrapes and re-coalesces the whole population for no reason and can hide a per-subject correctness gap. Reuse the shared **leaf** steps the sweep also calls — not its scheduler/orchestrator entry point.
Example: `IngestStreamSynchronizerService.activate(ingestNodeId, name)` (triggered by the `runOnReady` webhook / `POST /api/nodes/:nodeId/stream-ready`) upserts and deploys that one stream, reusing `upsertDiscoveredStream` + `ensureAssigned` + deploy, rather than invoking `SyncOrchestratorService.execute` for a single arrival.
Enforced: review.

**SVC-11** — Rule: Near-duplicate methods collapse to one. Two methods doing near-identical work do not both survive: one calls the other, or one is deleted and its callers reuse the survivor. A second method earns its existence by a real difference in *what it does*, never by the *context that calls it* — "it's the ingest path" is not a reason for a parallel re-implementation of an operation the discovery path already performs. (Method-level companion to SVC-05, which covers pure forwarders; this covers two independent implementations of the same operation, where neither wraps the other.)
Example: `recordLiveOnIngest` was deleted and its caller reused `upsertDiscoveredStream`, which already performed the same upsert-on-discovery (differing only in a couple of fields that became optional inputs).
Enforced: review.
...
Example: `StreamAssignmentService.assignToNode` throws when the repo returns null.
...
**INT-06** — Rule: A cluster MediaMTX stream lives on exactly the node it was **assigned to** — so any single-node operation against it (create pipeline, fetch stream details) MUST target that node's host, never a pick over the pool, which would hit a sibling replica and 404. The `media-nodes` service is handed the role + node id (`buildClusterPullPipeline(name, assignedNodeId, pullSource)`, `getStreamDetails(role, name, nodeId)`) and resolves it to a client via `NodeResolver.clientForNode(role, nodeId)`, which finds that node in the **live** set and throws when it is gone (ARCH-11) — no static-pool or round-robin fallback masks a missing node. Fan-out operations (delete, list) still sweep every active node. The cluster relay's pull source is resolved once by the caller (`StreamPipelineService.build`): an ingest-origin stream pulls from its `ingestNode` via `NodeResolver.getIngestRtspUrl(ingestNodeId, name)` (ADR-0013; the retired `${INGEST_RTSP_URL}` front is gone), any other stream uses its stored pullable source — the leaf `buildClusterPullPipeline` takes the resolved URL, never a nullable id (SVC-08). The same per-node addressing now applies to **ingest**: a stream is reserved on and published to one specific ingest node (INT-06 extends to ingest), placed least-loaded by `IngestPlacementService` (context-free `selectLeastLoaded`) at reserve time (`StreamReservationService`). Cluster assignment persists (`StreamAssignmentService.ensureAssigned` mutates an existing stream's `assignedNode`); ingest placement does not get its own persist step — it is a birth-time input folded into the `createReservation` insert, so it lives in the reserve/placement services, not the assignment service.
Example: `media-nodes/services/{pipeline,stats,topology}`; `NodeResolver.getClusterClientForNode(nodeId)` resolves the assigned host from live nodes (throws if absent) while `getActiveClusterClients()` returns the fan-out set (empty if none live); covered by `test/media-nodes/services/pipeline` and the node-threading test in `test/stream-inspection`.
...
**ERR-02** — Rule: Every log line names its subject (stream name, node ID, workflow name).
...
**DEVN-07** — DIR-10 is satisfied by `alerts/`, `nodes/`, `streams/`, and now `infrastructure/media-mtx/types/` (split into `v3/`, `stream/`, `metrics/` + flat shapes; the old 5-shape `media-mtx.types.ts` is gone). Remaining packers: `common/domain/types/event-payloads.types.ts` (3), plus several two-shape files. Apply DIR-10 on touch.
