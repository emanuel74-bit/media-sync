...
- Add an append-only ADR and the next stable `TOOL` convention rule; update `DOC-05`, agent
  instructions, and validation/tooling documentation to point to the canonical commands without
...
`TEST-01`, `TEST-02`, `TEST-05`, and `TEST-06`. This tooling and workflow decision requires a
new ADR under `DOC-01`; no existing ADR is superseded. ADR-0015 remains the governing decision for
