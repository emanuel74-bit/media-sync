...
export function generateStreams(count = 42): Stream[] {
    return Array.from({ length: count }, (_, i) => {
        ...
        return {
            ...
            status,
            metadata: {},
            isEnabled: status !== "inactive" || Math.random() > 0.3,
            lastSeenAt: ago(Math.random() * 2),
            lastSyncedAt: ago(Math.random() * 4),
            lastError:
                status === "error" ? "Connection timeout after 30s" : undefined,
...
export function generatePods(): Pod[] {
    ...
    return podIds.map((podId, i) => ({
        ...
        lastHeartbeatAt:
            podStatuses[i] === "inactive" ? ago(1) : ago(Math.random() * 0.02),
...
export function generateInspection(
    ...
    const tracks: StreamTrack[] = [
        {
            type: "video",
            codec: pick(videoCodecs),
            bitrate: rand(1500, 8000),
            width: w,
            height: h,
            fps: pick([24, 25, 30, 50, 60]),
        },
        {
            type: "audio",
            codec: pick(audioCodecs),
            bitrate: rand(96, 320),
            channels: pick([1, 2, 6]),
            sampleRate: pick([44100, 48000]),
            language: pick(["en", "es", "fr", "de", undefined]),
        },
    ];
    if (Math.random() > 0.7)
        tracks.push({ type: "subtitle", codec: "SRT", language: "en" });
    return {
        _id: crypto.randomUUID(),
        streamName,
        source,
...
export function generateInspectionHistory(
    streamName: string,
    limit = 10,
): StreamInspection[] {
...
export function addMockStream(data: {
    name: string;
    source: string;
    isEnabled?: boolean;
}): Stream {
...
export function updateMockStream(
    name: string,
    data: {
        source?: string;
        isEnabled?: boolean;
    ...
    if (data.isEnabled !== undefined) s.isEnabled = data.isEnabled;
...
export function assignMockStream(
    name: string,
    podId: string,
): Stream | undefined {
    const streams = getMockStreams();
...
export function rebalanceMockStreams() {
    const streams = getMockStreams();
    const pods = getMockPods().filter(
        (p) => p.status === "active" && p.tags.includes("cluster"),
    );
    const unassigned = streams.filter((s) => !s.assignedPod && s.isEnabled);
