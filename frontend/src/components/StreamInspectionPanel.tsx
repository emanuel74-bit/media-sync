  ...
  return (
    <div className="rounded-lg border border-border/50 p-3 space-y-2">
      <div className="flex items-center gap-2">
        ...
        {track.codec && (
          <Badge variant="secondary" className="text-xs font-mono-metric">
            {track.codec}
          </Badge>
        )}
        {track.language && (
          ...
          <>
            ...
            {track.fps != null && (
              <div>
                ...
                <p className="font-mono-metric">{track.fps}</p>
              </div>
            )}
            {track.bitrate != null && (
              <div>
                <span className="text-muted-foreground">Bitrate</span>
                <p className="font-mono-metric">{track.bitrate} kbps</p>
              </div>
            )}
          </>
        )}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <Badge
          variant={inspection.source === "ingest" ? "default" : "secondary"}
        >
          {inspection.source}
        </Badge>
        {isLatest && (
          <Badge
            variant="outline"
            className="border-status-healthy/30 text-status-healthy text-xs"
          >
            Latest
          </Badge>
        )}
        ...
        {typeof inspection.metadata.encoder === "string" && (
          <Badge variant="outline" className="text-xs font-mono-metric">
        ...
        <Card className="border-border/50">
          ...
          <CardContent>
            <Accordion type="single" collapsible className="space-y-1">
              {history.map((insp, i) => {
                const inspectionKey = `${insp.source}-${insp.inspectedAt}-${i}`;
                return (
                  <AccordionItem
                    key={inspectionKey}
                    value={inspectionKey}
                  ...
                  >
                    <AccordionTrigger className="py-2 text-sm hover:no-underline">
                      <div className="flex items-center gap-2">
                        <Badge
                          variant={
                            insp.source === "ingest" ? "default" : "secondary"
                          }
                          className="text-xs"
                        >
                          {insp.source}
                        ...
                        <span className="font-mono-metric text-xs text-muted-foreground">
                          {format(
                            new Date(insp.inspectedAt),
                            "MMM d, HH:mm:ss",
                          )}
                        </span>
                        <span className="text-xs text-muted-foreground">
                          {insp.tracks.length} tracks
                        </span>
                        {insp.lastError && (
